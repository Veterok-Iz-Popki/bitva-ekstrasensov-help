...
- Отзывы в React отрендерены дважды (desktop `hidden md:block` + mobile `md:hidden`, 25+25 карточек, идентичные тексты, в HTML каждый отзыв встречается 2 раза, в `innerText` — 1). Только зафиксировано, не исправлялось.
